...
if (isset($_SESSION["username"])) {
    ...
    if (isset($_POST["activate"])) {
        ...
        $portnr = $_POST["activate"];
        $time = $_POST["time"];

        $iduser = $_SESSION["id"];

        $endtime = date("H:i:s", strtotime('+' . $time . ' hours'));
        ...
        $query = "INSERT INTO tblaktivieren (dtAnfangszeit, dtEndzeit, dtDatum, fiBenutzercode, fiPort)
        VALUES (CURRENT_TIME , '$endtime', CURRENT_DATE, '$iduser', '$portnr')";

        mysqli_query($mysqli, $query);

        $query = "UPDATE tblPort SET dtPortstatus = 1 WHERE idPort = '$portnr'";
    ...
    <div><p>> <span>ERROR CODE</span>: "<i>HTTP 403 Forbidden</i>"</p>
        ...
        <p>> <span>ERROR POSSIBLY CAUSED BY</span>: [<b>execute access forbidden, read access forbidden, write access
                ...
                denied, infinite depth is denied, too many requests from the same client ip, not logged in</b>...]
        <p>> <span>SOLUTION</span>:<a href="../index.php?page=#login">LOGIN</a>
    </div>
